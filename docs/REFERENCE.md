...
**Postmark plan / email quota — ⛔ BLOCKED, all email testing PARKED (s87, 2026-06-27):** the inbound capture path lives or dies on the Postmark plan's monthly email count, and **Postmark counts every email that *arrives* at the address, not just deliberate captures** — a `200 "ignored, unauthorized sender"` reply still burns one received message, so spam / newsletters / Postmark's own notification emails to `@nospaces.xyz` drain the same bucket. **Both directions count** (each inbound forward = 1; each talkback reply, once active, = 1 more, so real use ≈ 2/capture). The free **Developer plan = 100/month** and we blew through it in one s87 test session (talkback wasn't even on) → inbound is now blocked until the 7/2/26 billing reset. **Can't upgrade to fix it:** Postmark won't let Farah buy the paid 10k/month (~$15) plan until they **approve the account** — the *same* pending approval that gates talkback (submitted 2026-06-02, chased s87). **So: no email-in testing at all until approval lands; Farah will pay for the 10k plan then.** The s87 screenshot-capture feature is therefore code-complete but its email path is **unverifiable for now** (the in-app flip / find-online / review chip are still testable on existing items — see HANDOFF). *If captures silently stop landing in future, check Postmark plan usage first — an over-limit/unapproved account blocks inbound invisibly (no error, the webhook just never fires).* Inspect what consumed the quota in Postmark → inbound server → Activity.
