...
**Screenshot a product, in-app (s87) — the Postmark-free capture path.** Things FAB → "screenshot a product" → pick/take a screenshot → it's downscaled (`downscaleImage`, also normalizes HEIC) + uploaded to Storage (`uploadMoodImage`, `mood-images` bucket — reused, just storage) so the card gets a real hosted image, then ONE Sonnet vision read (`/api/screenshot-product` → `readProductFromImage` in `api/_vision.ts`) pulls the product's **identity (name/brand/price) + look-tags + shot type** off the picture. Saves **live** to the board (no review gate — a deliberate in-app save is the signal), linkless (`url:null` → "find online ↗" recovers buy-back), and runs the cutout for a clean packshot. ~1¢ vision/screenshot, storage free. This is the in-app twin of the email screenshot path (`classifyEmailImage`) but URL-based + product-only; both share `_vision.ts`. **Beats the email path** (hosted image + cutout, no Postmark quota) and **works today** while email-in is parked. Multi-select supported (sequential, to respect the vision rate-limit).
