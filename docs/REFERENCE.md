...
- `POSTMARK_SERVER_TOKEN` — activates email talkback (replies to sender). ✅ set — talkback confirmed live s104 (Postmark account approved). Server API Token from the inbound server's API Tokens tab.
- `POSTMARK_FROM` — optional reply-from override
- `PUBLIC_BASE_URL` — optional; base URL for the talkback **undo** links (falls back to `https://nospaces.vercel.app`). Only set if the deploy domain changes.
...
### Email capture
Forward anything to `anything@nospaces.xyz`. AI finds every media item + saves as `want_to`. Photo attachments (incl. HEIC) work. **Big photos don't work via email** — Vercel caps at 4.5MB; Postmark always inlines full attachment; Gmail can't shrink. Use in-app "Add from a photo" instead (downscales to 1600px client-side). **Talkback** (replies to sender): **LIVE (s104)** — every save/failure gets a reply.

**Undo link (s104).** Every "saved" talkback reply ends with an undo link. `api/_undo.ts` mints an HMAC-signed token (user id + saved row ids + issued-at, signed with `EMAIL_WEBHOOK_SECRET`, 60-day expiry); `api/undo.ts` serves it. **GET shows a confirm page with a POST button; the row is only deleted on POST** — deliberately, so mail-client / security-scanner link *prefetch* (which does GETs) can't silently delete a save. The delete is scoped to `id IN (signed ids) AND user_id = signed user`. Undo on the media reply removes everything that email added (media rows + any products read off screenshots in the same message).

**Anti-hallucination guard on the text reader (s104).** `EMAIL_PROMPT` now forbids inventing a medium/creator/blurb for a title it doesn't actually recognize (the s104 bug: a 2025 book emailed as bare "title + author" was saved as *music* with a fabricated album bio). Unknown items must report `confidence:"low"` (→ land in the review inbox, not live) and leave `summary` null; a bare "Title by Name" must not default to `music` (a name is usually an author → book).

**Postmark plan / email quota (approved s104):** **Postmark counts every email that *arrives* at the address, not just deliberate captures** — a `200 "ignored, unauthorized sender"` reply still burns one received message, so spam / newsletters / Postmark's own notification emails to `@nospaces.xyz` drain the same bucket. **Both directions count** (each inbound forward = 1; each talkback reply = 1 more, so real use ≈ 2/capture). The free **Developer plan = 100/month**; the account is now **approved**, so the paid **10k/month (~$15)** plan can be bought — *confirm Farah has, or talkback + real use will blow the free cap fast.* *If captures silently stop landing, check Postmark plan usage first — an over-limit account blocks inbound invisibly (no error, the webhook just never fires).* Inspect what consumed the quota in Postmark → inbound server → Activity.
...
**Screenshot a product, in-app (s87) — the Postmark-free capture path.** In the **"save a product" card** (Things FAB → save a product), "screenshot a shop page" sits beside the paste-a-link input. Pick/take a screenshot → it's downscaled (`downscaleImage`, also normalizes HEIC) + uploaded to Storage (`uploadMoodImage`, `mood-images` bucket — reused, just storage), then ONE Sonnet vision read (`/api/screenshot-product` → `readProductFromImage` in `api/_vision.ts`) pulls the product's **identity (name/brand/price) + look-tags + shot type + a normalized bounding box** of just the product's photo. The client **crops the screenshot to that box** (`cropBlobToBox`, +4% pad) and hosts the crop, so the card is the product alone — not the whole page (browser bars / shop header / price text); falls back to the full shot if there's no box. The card then shows the prefilled fields for a tweak + an **"add to a plan?"** selector (standalone product / candidate on an existing intent / first option of a new intent — `readScreenshotToFields` + `saveComposedProduct`). Saves **live** (no review gate — a deliberate in-app save is the signal), linkless (`url:null` → "find online ↗" recovers buy-back), runs the cutout for a clean packshot. ~1¢ vision/screenshot, storage free. In-app twin of the email path (`classifyEmailImage`); both share `_vision.ts`. **Beats the email path** (hosted+cropped image, no Postmark quota) — prefer it when the quota matters.

**Things via email (s66):** two ways in. (1) Send a product link to **`things@nospaces.xyz`** (or `shop@`/`want@`) — routed by recipient local-part (`isThingsAddress`, env `THINGS_EMAIL_LOCALPARTS`), lenient (any readable link), no Anthropic call. (2) **The normal media inbox also auto-handles products:** if a (non-photo) email yields no media but contains a *product-like* link (`scrapeProduct`'s `productLike` = JSON-LD Product / `og:type=product` / has a price), it's saved to the board instead of "nothing found" — so the regular address "just works" for things too. Strict productLike gate so a forwarded article doesn't become a card. Both paths: scraped via shared `api/_scrape.ts` (**free**), saved as `type:'thing'`, dedup by URL, first scrapable link wins. `extractEmailUrls` reads links from both plain text AND HTML `href`s (forwarded shop emails are often HTML-only). Domain inbound forwards all local-parts to `/api/email`, so no extra Postmark setup. Talkback confirmation replies are live (s104), so an emailed thing now also gets a reply (with an undo link).

**Articles via email (s115) — read-later bookmarks, `type:'article'` in the media library.** A bare bookmark, not a saved copy: title/byline/publication/thumbnail/link only (paywalls block full text, but og-tags meant for social previews usually survive them). On the universal capture-all address, a link that isn't `productLike` is next checked for `articleLike` (`scrapeProduct`'s `og:type=article`, or an `article:author`/`article:published_time` meta tag) before falling through to the paid AI media reader — so a shared article link saves for **free**, same shared `api/_scrape.ts`, same dedup-by-URL pattern as things (`captureArticle()` mirrors `captureThing()` in `api/email.ts`). The page's dek/description is stored as `metadata.capturedBlurb` so it reuses the item sheet's existing "about this" collapsible. Deliberately no genres/vibes/verdicts/gaps for articles — `want_to` (saved) → `done` (read) is the whole lifecycle, no reaction. Getting a link out of a publisher app (New Yorker/Apple News/etc) on iOS still needs the share→silent-email Shortcut planned for Things (not yet built) — until then, forward the link by hand.
