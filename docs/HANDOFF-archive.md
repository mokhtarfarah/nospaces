...
### Session 121 (2026-07-24) — article list rows show the magazine; email account-lookup hardened. Free (frontend + free lookup path, no API). Pushed to `main` ([82aa33f], [83eea1c]).

**(1) Library list view — magazine before the author ([82aa33f]).** Article rows in the **list** layout showed only `title · creator` (author). Farah asked to surface the **publication** itself, ideally before the author. The grid tiles already compute a source name (`metadata.siteName`, falling back to the URL hostname) for the overlay band — so I pulled that inline logic into one shared helper **`articleSourceName(item)`** (`src/screens/LibraryScreen.tsx`) and used it in both places, so grid + list can't drift. List rows now read **`title · magazine · author`** (e.g. *How Lizzo Became One of Pop Culture's Great Flops · The Atlantic · Spencer Kornhaber*); only articles are affected, other media untouched. Typecheck clean, **125 tests green**. **Not visually verified — the no-auth sandbox has empty library (no backend), so no article to render; Farah to eyeball on real data.** Two edge cases flagged for her eye: (a) an article with no captured `siteName` → does the hostname fallback read cleanly (e.g. "newyorker" vs "The New Yorker")? (b) one library item whose *title* already starts with the author's name ("Jonathan Coe · Where was the drum kit?…") may now double up as `… · The Guardian · Jonathan Coe` — that's a capture-time data quirk, not the layout.

**(2) Email capture "user not found" — root-caused as a transient blip; hardened + made self-diagnosing ([83eea1c]).** Farah forwarded an article to `save@` and got the talkback: *"couldn't match it to your account… (user not found)."* The endpoint had run (webhook secret + Postmark + talkback all fine), so the only failure was `matchedUser` coming back null in the sender→account lookup (`api/email.ts`). **Investigated:** matching logic unchanged since the endpoint's creation (June 1), so not a code regression → environmental. My first hypothesis was **unpaginated `listUsers()`** (defaults to first 50 accounts; every Google sign-in makes an auth row, so real owners could get pushed off page 1) — but **Farah checked the dashboard: only 2 users**, which rules that out (both always on page 1). **She re-sent → it saved.** That self-resolution rules out the two *persistent* causes (bad service-role key, wrong from-address), leaving a **transient `listUsers()` hiccup** (Supabase admin API blip / cold start) that the old code mislabeled as "user not found." **Shipped anyway (both correct regardless):** (a) **paginate the lookup** (walk all pages, `perPage:200` — harmless safety net if users ever grow past 50); (b) **split the two failure modes** that both collapsed into "user not found" — a lookup *error* now replies *"couldn't reach the account database… try again in a minute"* (names a transient/credential issue, tells the reader to retry), distinct from a genuine *no-match* which now says *"couldn't match [email]… send from the email you sign in with."* So if this recurs, the reply names the real cause instead of sending us guessing. **Honest:** the retry is what saved the article, not the code — but the next occurrence will be diagnosable + tell Farah to just re-send. Cost: **free** (account-lookup path, no Anthropic). Typecheck (src + api) clean, **125 tests green**.

*Lesson: an unpaginated `listUsers()` + a swallowed admin-lookup error both surface to the user as the same misleading "user not found." When a capture bounces with that message, first action is **re-send once** — a transient admin blip clears on retry; if it persists, the new wording distinguishes service-role-key vs from-address.*
...
**Follow-on fix — horizontal drift (same session, two passes [4a435e4] then [ef393b2]):** Farah caught the app drifting sideways when the filters panel opened (wide verdict labels like `comfort 28 · would revisit 20 · guilty pleasure 13…`). **First pass [4a435e4]** nudged the panel on-screen with a `useLayoutEffect` + `maxWidth: calc(100vw-24px)` — but she reported drift *still* there, because the Chrome preview can't reproduce it. **Root-caused in [ef393b2] — two causes, both iOS/spec-specific:** (1) the **content scroll container** (`listRef`) had `overflowY:auto` with `overflow-x` defaulting to `visible`; the browser promotes that to `auto`, so the region itself scrolls sideways whenever a child overflows right → set **`overflowX:hidden`**; (2) the panel nudge used a **CSS transform**, but **iOS Safari still counts a transformed element's original box in scrollable width** — looks aligned, page still drifts → switched the nudge to a real **`left`** offset, which every browser measures for overflow. Verified in-browser: with the filters button pushed to the right edge and a wide panel open, the main scroll container stays `scrollW==clientW` (no drift) while the category-tab row still scrolls internally as intended. *Lesson: Chrome ignores transform-overflow and auto-promotes overflow-x silently — reproduce drift by measuring the scroll container's `scrollWidth`, don't trust the preview looking fine.*

**Then (same session, [a24c782]) — count placement + badge de-emphasis (Farah's follow-up once drift was fixed).** (1) The result count was a bare number cramped on the sticky nav row → **moved to a quiet labeled subline under the title** (`countLabel`: "69 books" for a single medium, "N items" for all/search, "N to review"). This is the s119 "count is wrong = placement, not math" fix, now shipped. (2) The active-filter badge was a filled black pill that shouted → **de-emphasized to a plain muted-grey number** beside the italic "filters" label (the italic ink already carries the on-state). Typecheck clean, 125 tests green; verified in preview (mobile): subline under the title, badge renders as a quiet grey `1` when a filter is active.

**Relation to the queued menu-nav pass:** the **filter-bar declutter slice is now fully done** (one filters menu + count-as-subline + quiet badge). What remains of the menu-nav pass: the **per-medium type-row** (music/films/books/tv still its own row) and the **dual-nav sub-tab** placement — menu-nav stays queued (Tier 1) for those.
...
**Then built the palette-protected skin gate** — the s118 "colour story leaks model skin tone" plan, exactly as decided (removed from ROADMAP → Things board polish). The colour-story ribbon on the Things taste page pulled warm browns straight off a **model's skin** in on-model/lifestyle/mood shots, so a monochrome board showed flesh-brown swatches. Can't gate on flesh RGB — Farah's signature camel/tan/leather sits on top of skin in RGB. Solution shipped:
- **`src/lib/palette.ts`** — `sampleBoardColors` now takes `BoardImage[]` (`{src, clean}`) and runs **two passes**: (1) sample the **clean packshot cutouts** (transparent PNG, zero skin — the cutout *is* the garment) → their colours become a **known-good palette**; (2) sample the worn/mood shots with a **skin gate** — `imageColors(src, skinGate)` drops a pixel when `isSkin(r,g,b) && !nearKnownGood(...)`. So skin (never in a cutout) is dropped, camel/leather (confirmed by a cutout) is protected, worn-only accessories keep their colour minus surrounding skin. `isSkin` = the broad Kovac warm-flesh rule; `nearKnownGood` = Manhattan dist < 45. Both exported for testing.
- **`src/screens/ThingsScreen.tsx`** — the call site (`boardImages`, was `imageSrcs`) tags each image: a product with a live cutout → `{clean:true}` (anchors the palette); its raw on-model/lifestyle photo or a mood image → `{clean:false}` (gated).
- **`src/lib/palette.test.ts`** (new, 7 tests) — pins the exact bug: skin-brown reads as skin AND drops when unconfirmed; camel confirmed by a cutout survives; the "no cutouts → nothing protected" **known gap** is a documented test.
