...
**Follow-up (same session):** added a **clear** button to the feed (deletes the user's whole capture log) — needs an RLS *delete* policy run in Supabase (the table migration was already run + the feed confirmed working in-app). Silently no-ops until the policy is added.
