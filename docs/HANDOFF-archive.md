...
**Follow-on fix (same session, [4a435e4]):** Farah caught the dropdown panel spilling past the right edge on her phone (wide verdict labels like `comfort 28 · would revisit 20 · guilty pleasure 13…`), which widened the page and let **the whole app scroll sideways**. The popover was anchored `left:0` from a button that sits far right on the nav row. Fixed by keeping the left-align but adding a `useLayoutEffect` that **nudges the panel horizontally by just enough to keep both edges on-screen** (the button roams along the scrolling row — far right with all category tabs, far left on an empty library), plus `maxWidth: calc(100vw - 24px)` as a hard cap. Verified with a 600px row injected (no horizontal page scroll), at a 260px viewport (panel nudged fully visible), and on the empty library (left-aligned, on-screen).
