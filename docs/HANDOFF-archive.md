...
Three fixes shipped on `main` (07f3bee, 6b4b4ba), **106/106 Vitest** (98 + 8 new) + typecheck (app+api) + lint + genre-sync green each push. No Anthropic cost change — all work is on the free public catalog APIs (iTunes/Deezer/TMDB/Open Library/Apple Books), browser/serverless-direct. Not browser-verified: every touched endpoint is auth-gated (`describe`/`art` need a logged-in session) or key-gated (TMDB/Apple keys live only on Vercel), and the local no-auth library is empty — so verified by curl against the live APIs + unit tests + the gate. Needs a real spot-check on the deployed preview.
