...
- **In-app "screenshot a product" shipped (s87) — the Postmark-free capture path, testable NOW.** Things FAB → "screenshot a product" → hosts the shot + one vision read for name/brand/price + tags → saves live to the board with image + cutout (`api/screenshot-product.ts`, `readProductFromImage`). Verify the *read quality* on real walled-shop screenshots (Farfetch/miista) on a phone; if junky, tune `readProductFromImage`'s prompt in `api/_vision.ts`. *Possible follow-on if it proves out: a matching in-app "screenshot media" path so the whole capture story is Postmark-independent.*
