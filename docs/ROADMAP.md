...
- **"New music tuesday" as a music-only filter chip** — currently a toggle in the Library status row (only shows in the music category). Farah wants it folded into the filter sheet as a proper filter category for music, rather than sitting in the menu bar taking space. Minor UI move. *Trigger: next Library polish pass.* (`LibraryScreen.tsx` status row ~619 + `FilterSheet`.)
