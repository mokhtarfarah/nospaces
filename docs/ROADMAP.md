...
- **Discover redesign — not in final form.** Redesign shipped (bigger covers, blurb hero, ink save chip, no-repeat logic) but Farah flagged it still needs work. Two concrete findings feed the active editorial queue: (1) hard-gated behind the taste profile — no profile = dead-end empty state; (2) cover sizes inconsistent (music 72px vs others 56/84px) so rows don't align. *Trigger: the editorial polish pass (in HANDOFF Next session).*
