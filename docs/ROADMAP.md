...
#### Screenshot-capture for the walled shops — DECIDED, ready to build (s86)
The recurring 403 (Farfetch, Net-a-Porter, miista = Akamai/DataDome/Vercel-checkpoint bot-walls) **can't be beaten by headers** — a serverless IP with no real browser loses forever. We decided **not** to fight the wall: go around it with a screenshot, which never scrapes. Rejected the scraper-API (ScraperAPI/Zyte/Cloudflare browser-rendering) route — works, but a new vendor + bill + ongoing arms race, and screenshot covers more (works inside shop *apps* too). The old "no buy-link" blocker is solved cheaply (see #4 below). **Mental model the build must reinforce: share the *link* by default (free scrape, best data when it works); the few that fail tell you (talkback + miss inbox) → screenshot *that one*. Screenshot is the rescue, not the default.**

The build (one feature, ~1¢ vision per emailed screenshot is the ONLY new cost; everything else free):
1. **Screenshot → live capture via email.** An image emailed to `save@` gets ONE Sonnet vision read that classifies **product vs media** and extracts fields (product: name/brand/price + look-tags in the same call; media: title/creator/type/year). Route product→board, media→library. Process **non-inline** image attachments (no `ContentID`) so a deliberate screenshot is read but forwarded-shop-email decoration (inline thumbnails/swatches/pixels) is not — and so a screenshot survives even when a link is also present (softens today's "link wins → discard attachments" rule to "link wins *only if it yields a product*, else use the image").
2. **Review-flag construct, consistent across BOTH domains, gated on CONFIDENCE not domain.** Rule: **review is for the machine's uncertainty, not yours.** A single deliberate capture (shared link, one screenshot) lands **live** — board or library, no gate. Only **bulk newsletter extraction** + **low-confidence vision reads** get `metadata.review = true`. Things need the `review` flag + a board "for review" filter mirroring the Library's. The review view is an **ignorable filter, never a blocking gate or a guilt badge** (respects the "saving is the signal" soul rule — don't tax deliberate saves).
3. **media↔thing flip — CRITICAL, ship with it.** A one-tap "this is actually a thing / actually media" that moves an item across domains (changes `type` thing↔film/book/music/tv + reshapes metadata). This is the real safety net for misroutes — cheaper + less annoying than any review gate (false-positives are cheap to undo; don't make every true-positive wait to prevent them).
4. **Free "find online ↗" on board cards.** Builds a search URL from brand+title (`google.com/search?q=…`) — a plain link, **no scraping, no API, no cost** — so a screenshot-captured thing (which has no stored URL) still gets a one-tap path back to buy. This is what unblocked the old "linkless board entry is half-dead" concern.
5. **Failure copy nudges the fallback.** Both the talkback reply AND the miss-inbox row for a failed link should say *"screenshot it and send that instead"* so the rescue is obvious in the moment.
...
**Why this exact shape — rejected alternatives (so we don't re-litigate):**
- **Beating the wall with better headers** — rejected. Already send a full real-browser UA; Farfetch still 403s. Akamai/DataDome fingerprint TLS + JS-execution + IP reputation, not the UA. A serverless IP with no real browser loses *permanently* — not a tuning problem.
- **Requiring "screenshot + link together"** (to keep the buy-link) — rejected. On iPhone, sharing a *page* gives a link and sharing a *screenshot* gives an image — **two different mechanisms**; you can't do both in one gesture without manually composing an email. That destroys the whole one-tap value. So screenshot is link-*less* by design, and #4 (find-online) recovers buy-back instead.
- **OCR the URL off the screenshot's address bar** — rejected. iOS Safari shows only the **domain** ("farfetch.com"), not the product path, and collapses to domain on scroll. Gives the shop, not the item — useless for buy-back. (Don't re-propose this.)
- **Screenshot *everything* (drop links)** — rejected. The link is better *when it works*: real buy URL, price, clean product image, and **free** (no vision). Most shops scrape fine. So link stays the default; screenshot is only the rescue for the ones that fail — and failures are now visible (talkback + the shared miss inbox), so you never have to guess upfront which will fail.
- **A universal/mandatory review gate** — rejected. It taxes every *deliberate* save to prevent the rare misroute, and a gate (nothing exists until you approve) violates "saving is the signal". False-positives are cheap to undo, so we **correct-after** (the flip) rather than **approve-before** (a gate). Review is reserved for the machine's own uncertainty (bulk newsletters, low-confidence vision).

- **Known scraper-hostile shops** — **miista.com** (HTTP 429 "Vercel Security Checkpoint") and **farfetch.com** (HTTP 403, Akamai — confirmed s86, even with a real-browser UA) can't be read server-side. The screenshot path above is their fix. Keep as real-world test cases.
...
- **Verdict reshape (parked, Farah s75)** — on the media item sheet, "verdict" gets its own genre/vibe/**verdict** tag-row, but it largely repeats the reaction already shown (`· loved it`). Farah registered it "isn't totally working" but parked it — the reshape isn't settled (drop the redundant row? fold verdict into the reaction? make it carry something the reaction doesn't?). *Trigger: Farah comes back with what she wants verdict to be.* (`ItemActionSheet.tsx`, the genre/vibe/verdict rows ~`:708`.) **Tackle with the music-clutter item — same area.** *Also relates to the s85 filter "idea 4" (parked): in the filter card, `verdict` (your take: in-rotation/comfort/guilty-pleasure) is a different mode than genre/vibe (descriptive) — consider pulling verdict up as quick-chips near status and leaving genre/vibe/region as the deep tag browse. Same "what should verdict be" question, two surfaces.*
