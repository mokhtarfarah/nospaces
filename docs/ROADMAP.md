...
- **Known scraper-hostile shops (test cases for the fingerprint work)** — **miista.com** returns HTTP 429 "Vercel Security Checkpoint" (a JS bot-challenge a plain fetch can't pass), so its product pages can't be read server-side via email/shortcut capture (confirmed s86). Use it as a real-world test case when the scraper-403/fingerprint work happens. Until then, save@-capturing a Miista link silently fails to scrape (the link is found, the page just won't yield). Screenshot-capture (above) is the likely fallback for shops like this.
