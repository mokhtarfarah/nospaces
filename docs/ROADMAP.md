...
- **Mood masonry rebuild (s80)** — the moodboard wall is now **JS shortest-column masonry** (`MoodWall`, `ThingsScreen.tsx:1882`): newest-first across the top *and* gapless (Farah wanted both; CSS gives one or the other). Tiles report aspect ratio on `onLoad`; layout settles. **Confirm:** gapless + newest spreads across the top, and the column-shuffle as images stream in / lazy-load isn't distracting. *If the settling annoys: store each image's dimensions at save time (more plumbing) so the layout is right on first paint.*
- **Cold-start + coherence pass (s81)** — first-time-user polish, all on `main`: warmer empty states (library + things, kept the wink, dropped the "you loser" insult); a blurred **"locked preview"** on the empty taste page (shows the payoff before it's earned) + its missing domain switcher; a more visible media/things switcher (bolder active, underlined inactive); Things chrome lowercased to one voice; the cover **reaction badge** (☺ loved / ✓ done) replacing the undecodable ink-vs-grey dot; the **deciding-card grid cover** restored (photo fills the tile, frosted overlaid title, single line + ellipsis); and **"wishlist"** as the single name for the board (was double-named "board"/"wishlist", and "board" collided with "mood board"). **Verify on a real account:** the cover badges and the deciding-card cover on live data (the empty harness can't show them).
...
- **Vary the AI voice by surface (s81)** — today one shared voice block (`api/_humanizer.ts`) covers every prose generator, so the same lyrical register shows up everywhere and risks becoming a formula. Give each surface its own **register on top of the shared base**: warm/observational on the taste read, terse on discover, decisive on compare/deciding. Cheapest of the taste work and **ungated — can go first**. While here, confirm the Things prose endpoints (`api/things-taste.ts`, `things-compare.ts`) actually import the humanizer base. (Extends the standing humanizer principle at the top of this file.)
