...
- **Colour-story ribbon (s77)** — the editorial taste tab's colour ribbon is browser-only and was never eyeballed. Samples real pixels client-side (`src/lib/palette.ts`) and **hides itself below 3 colours**, so the failure mode is a *missing* ribbon — most likely a CORS block on Supabase `mood-images` URLs (tainted canvas → skipped). *If no ribbon: check the bucket sends `access-control-allow-origin`, or sample via the same-origin `/api/thing-image` proxy.* **Known bug (Farah, s80): the ribbon reads too gray because it counts product/tile backgrounds.** The sampler drops near-white (`mn > 232`) and near-black (`mx < 18`) but **not gray** (`palette.ts:67-68`), so mid-gray studio backdrops, padded shots, and the cutout tile all count as a colour — and the bucket-averaging/merge step further drifts mixed hues toward gray. **Fix: add a saturation floor** — drop low-saturation (near-neutral) pixels alongside white/black in `imageColors`, e.g. skip when `(mx - mn) / mx` is below ~0.12. Optionally weight remaining buckets by saturation so true colour outvotes muddy neutrals. Also re-confirm **column-major masonry** order (newest down the left column) is acceptable — if she wants strict chronological back, do JS shortest-column masonry instead of CSS columns.
