...
- **Screenshot-capture for a thing (the seamless mobile fallback)** — when a shop link won't scrape (Cloudflare etc.), let the user snap/pick a screenshot and AI fills it, mirroring media's `processImageFile` photo-identify (the client pattern already exists in `AddScreen`). **Parked s71:** a thing has no catalog, so a screenshot gives name/price/image but **no buy link** — and a linkless board entry is half-dead (hand-adding the link on mobile is the exact pain we're escaping). Making it work like media (identify → fetch the real source URL + info) needs **reverse product web-search** to recover the link = the **expensive web_search API + unreliable** (wrong colour / resale / blog). *Trigger: web-search-to-recover-link becomes acceptable on cost + accuracy, OR a cheaper product-lookup source appears. The s71 scraper-fingerprint upgrade is the cheap reliable lever meanwhile.*
